...
test("連携ラインは両端が稼働中/エラー対応のときだけ点灯する", async () => {
  const { COMMAND_DEPARTMENTS: depts, isConnectionActive } = await import("./commandCenter");
  // 市場調査部(working) × システム開発部(error) → 点灯
  assert.equal(isConnectionActive(depts, { from: "market_research", to: "development" }), true);
  // 企画部(working) × 市場調査部(working) → 点灯
  assert.equal(isConnectionActive(depts, { from: "market_research", to: "planning" }), true);
  // 営業部(done) × サポート部(working) → 消灯
  assert.equal(isConnectionActive(depts, { from: "sales", to: "support" }), false);
  // 未知の部署ID → 消灯
  assert.equal(isConnectionActive(depts, { from: "unknown", to: "support" }), false);
});

test("全部署に丸アイコンが定義され、未知IDは汎用アイコン", async () => {
  const { COMMAND_DEPARTMENTS: depts, DEPT_ICON, deptIcon } = await import("./commandCenter");
  for (const d of depts) {
    assert.equal(typeof DEPT_ICON[d.id], "string");
    assert.ok(DEPT_ICON[d.id].length > 0);
  }
  assert.equal(deptIcon("unknown"), "🏢");
});
