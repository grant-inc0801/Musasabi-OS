...
test("Mock検索は市区町村ごとに8件以上の店舗を返し、8項目が全て埋まる", () => {
  const results = provider.search({ prefecture: "大阪府", cities: ["高槻市", "茨木市"] });
...
test("Mock検索は決定的(同じ条件は同じ結果)で、空・空白の市区町村は無視される", () => {
  const q = { prefecture: "東京都", cities: ["新宿区", "", "  "] };
  const a = provider.search(q);
  const b = provider.search(q);
  assert.deepEqual(a, b);
  assert.equal(a.length, 1);
});

test("summarizeCallList は合計・市区町村別・ジャンル別・デリバリー数を集計する", () => {
  const results = provider.search({ prefecture: "大阪府", cities: ["高槻市", "茨木市"] });
...
test("callListToRows は見出し行+店舗行を8列で返す", () => {
  const results = provider.search({ prefecture: "京都府", cities: ["京都市"] });
...
test("buildXlsx は正しいZIP構造の .xlsx バイト列を生成する", () => {
  const rows = callListToRows(provider.search({ prefecture: "北海道", cities: ["札幌市"] }));
