...
- Voice EngineのSTT出力(テキストのみ)とVoice Analysisの入力(話者・タイミング付き
  セグメント)の間の話者分離(ダイアライゼーション)は未実装(次フェーズ Issue #182)
- FileMaker/Zoom Phoneは実サーバー未接続(Mockアダプタのみ)。連携準備UIは
  次フェーズで追加予定(docs/ARCHITECTURE.md 第4.3章)
...
- 話者分離を実装し、実通話からVoice Engine→Voice Analysisへの直結を実現する(#182)
- 実FileMaker/Zoom Phone連携準備UIの追加(docs/ARCHITECTURE.md 第4.3章)
- 実VOICEVOX/whisper.cppとの接続検証(#183)
