...
5. **Voice / Analysis**
   - ~~話者分離(ダイアライゼーション)の実装~~ — **実装完了(2026-07-04、Issue #182)**。
     `packages/voice-analysis` に `DiarizationBridge` を追加。Zoom Phoneの発信/
     着信チャネル分離を前提とした決定論的な話者分離で、Voice EngineのSTT出力
     (`TranscriptChunk`相当)を`generateCallSummary`にそのまま渡せる
     `TranscriptSegment[]`に変換する。詳細は下記
   - VOICEVOX/whisper.cpp 実エンジンとの接続検証(Issue #183)は**未着手**。
     実エンジンプロセス(VOICEVOX Engine、whisper.cppサーバー)がこの開発環境に
     存在せず、ネットワーク呼び出しの実地検証ができないため、Windows実機/実エンジン
     環境での作業として保留する

### 話者分離ブリッジ(`DiarizationBridge`、Issue #182)

`packages/voice-engine`のSTT出力(`TranscriptChunk`: テキスト+isFinal+timestampMs)と
`packages/voice-analysis`の入力(`TranscriptSegment`: 話者+テキスト+タイミング+
発話時間)の間には変換層が存在しなかった。これを`packages/voice-analysis/src/
DiarizationBridge.ts`で埋めた。

- **話者分離方式**: 声紋推定等のLLM/ML的手法ではなく、Zoom Phoneの発信/着信チャネル
  分離のようにチャネルごとに話者が既知であることを前提とした決定論的な方式
  (Development Bible「Deterministic behavior before LLM behavior」)
- **発話時間の見積もり**: STTプロバイダから実際の発話継続時間は取得できないため、
  `packages/voice-engine`の`MockTtsProvider`と同じ文字数ベースの概算値
  (1文字=120ms)を用いる。チャンクの`timestampMs`(認識完了時刻、両チャネル共有の
  通話タイムライン上の時刻)を発話終了時刻とみなし、そこから見積もり時間だけ遡って
  開始時刻を算出する
- **結合テスト**: `DiarizationBridge.integration.test.ts` で、2つの
  `MockSttProvider`(rep/customerチャネル)から生成した`TranscriptSegment[]`を
  `generateCallSummary`にそのまま渡せることを確認済み(Issue #182 受け入れ基準)
