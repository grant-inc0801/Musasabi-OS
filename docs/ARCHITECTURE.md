...
**現状**: Epic β-001 の Phase 1・2・8(`apps/desktop`)は、この方針転換より前に
**Electron** で実装され、動作検証(ビルド・ユニットテスト)済みの状態で `main` に
統合されている。これは**レガシー/暫定実装**であり、目標アーキテクチャではない。
`apps/desktop` の Tauri への移行は次フェーズの最優先事項(優先順位1)とする。
...
### Phase 1 — Windows デスクトップアプリ化(`apps/desktop`)【Electronで実装済み・レガシー】
**この実装はTauri方針決定(第0.2章)より前のもので、Electronベースの暫定実装として
mainに統合済み。Tauriへの移行は次フェーズ優先順位1(第9章)で行う。**
...
1. **Tauri Desktop安定化** — `apps/desktop` をElectronからTauriへ移行し、Phase 1/8相当
   (メインウィンドウ、トレイ常駐、自動起動、Sales Workspace読み込み)をTauriで再実装する
2. **MUSAアバター表示基盤** — アバターのオーバーレイウィンドウ・状態機械をTauri上で再実装
   (`packages/avatar-2d` のロジック自体はフレームワーク非依存のため再利用可能)
3. **Sales Workspace** — Tauri側からの読み込み確認、UIの継続改善
4. **FileMaker / Zoom Phone 連携準備UI** — 実アカウント接続は行わず、Settings画面に
   連携準備UI(接続ステータス表示、認証情報保存インターフェースの型のみ、本番用
   Provider Interfaceの雛形)を追加する。詳細は第4.3章
...
**実装する:**
- 本番接続用の Provider Interface(`FileMakerAdapter`/`ZoomPhoneAdapter` は
  Phase 4/5 で既に定義済み。これをUIから見える形に整理する)
- 接続ステータスの型・表示: 未接続 / 設定待ち / Mock接続中 / 本番接続準備済み / エラー
- 認証情報「保存インターフェース」のみ(実際の値は保存しない。プレースホルダー/
  ダミー値のみ許可)
- Sales WorkspaceのSettings画面に連携準備UIを追加

**禁止:**
- 実FileMaker Data API / Zoom Phone APIへの接続を試みること
- ダミー以外の実APIキー・OAuthトークンの保存
- FileMaker本番DBへの書き込み
- Zoom Phone本番APIへのアクセス

Mockアダプタ(`MockFileMakerAdapter`/`MockZoomPhoneAdapter`)は維持し、
「Mock接続中」ステータスの動作確認に使う。
