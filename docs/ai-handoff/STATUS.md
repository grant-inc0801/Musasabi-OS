...
## 直近の完了内容(第32弾: 通知フィルタ+保管庫並び替え)
通知センター実イベントにレベルフィルタ(すべて/情報/要対応/エラー・既読トグル併用可)。
保管庫一覧に並び替え(新しい順/古い順/サイズ順/タイトル順/出所順・検索併用可)。
E2E 10/10・0エラー。

## それ以前の完了内容(第31弾: 会議連結E2E収載+ビューア読み上げ)
CI収載E2Eに chat-meeting.spec(会議→実行 この結論→承認→完了)を追加し5スペックに。
保管庫ビューアに🔊読み上げ(閉じる時に停止)。E2E 5/5・0エラー。

## それ以前の完了内容(第30弾: 会議→実行連結+索引カバレッジ)
チャット「実行 この結論」で直近会議の結論をそのまま実行(会議→実行がチャット完結)。
Company Brain 意味検索ノートに保管庫索引カバレッジ(X/Yチャンク・Z文書)表示。
E2E 5/5・0エラー。

## それ以前の完了内容(第29弾: チャット会議コマンド+E2E拡充)
チャット「会議 ◯◯について」でAI会議開催→結論返信→議事録保管庫保存。CI収載E2Eを
4スペックへ拡充(forecast/scheduler-curation追加)。E2E 6/6+4/4・0エラー。

## それ以前の完了内容(第28弾: 保管庫全文ビューア+会議結論の実行連携)
