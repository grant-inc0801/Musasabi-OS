...
## 2026-07-14 — 本番実装第25弾: 保管庫文書の編集(タイトル・タグ)

### 実装内容(ノンストップ続行)
- **`updateVaultDocument`(vaultStorage.ts)**: タイトル・タグの編集(本文は不変)。
  タイトル変更時は文書IDを改版し、旧チャンクは次回の索引更新で prune → 新タイトルで
  再索引される(brainRag の既存機構に追従・検索が新タイトルを引ける)
- **保管庫ページに行内編集UI**: 各行「編集」→ タイトル/タグ入力(読点・空白区切り)→
  保存/キャンセル。一覧に 🏷タグ表示、絞り込み検索はタグにも一致
### テスト結果
- E2E 7/7: 編集→保存でタイトル/タグ更新(本文不変・ID改版)→🏷表示→タグで絞り込み→
  キャンセルは無変更。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第24弾: AI司書の定例整理提案(スケジューラ統合)

### 実装内容(ノンストップ続行)
- **AgentSchedule に kind "vault_curation" を追加**: 保管庫の整理候補の定例提案
  — proposeVaultCuration を定例実行し、候補があれば**要対応通知(warn)**+Brain記録。
  **提案のみで削除は一切しない**(削除は保管庫ページで候補ごとに人間が承認した場合のみ)
- **スケジューラUI**: テンプレート「保管庫の整理提案(AI司書)」+🧹バッジ。
  毎週などの定例で「保管庫が散らかったら教えてくれる」運用が可能に
### テスト結果
- E2E 7/7: テンプレ登録→🧹バッジ→今すぐ実行→「AI司書(決定論・提案のみ)」で
  候補1件(40日前のAI成果物・理由付き)→データ無傷(削除なし)→要対応イベント記録。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第23弾: チャットヘルプ+実未読バッジ

### 実装内容(ノンストップ続行)
- **チャット「ヘルプ」コマンド**: 「ヘルプ/help/使い方/何ができる」で使えるコマンド一覧
  (実行・承認/保管庫検索/今日何した?/接続状況/通常質問のRAG回答)を即答
- **Mission Control ヘッダーの🔔を実データ化**: Mockの承認数から**実未読イベント数**
  (unreadAppEventCount)へ変更し、クリックで通知センターへ遷移
### テスト結果
- E2E 3/3: ヘルプ一覧(4コマンド+RAG説明)→🔔=実未読2件→クリックで通知センターへ。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第22弾: Automation結果の保管庫保存+診断チャット連携

### 実装内容(ユーザー指示「いけるとこまで止まらず実行して」)
- **Automation ルーチン結果の保管庫自動保存**: 再実行の最終ステップ完了時に、
  実行サマリー(ルーチン名・手順・累計実行回数)を保管庫(source: agent)へ保存し
  実イベントにも記録 — 成果物ループの最後の未接続箇所(操作記録)を接続
- **診断のチャット連携**: 「接続状況は?」「ローカルAIの状態」「診断して」で
  ローカルAI5連携を実診断し 🟢/🟡/🔴+💡ヒント付きで即答(isDiagnosticsQuery/
  buildDiagnosticsReply・決定論)
### テスト結果
- E2E 6/6: チャット診断(LLM=🟡代替・5連携表示・💡ヒント)→ Automation再実行→
  保管庫に「自動化実行: <名前>」(手順+累計3回)+実イベント記録。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第21弾: ローカルAI連携の状態診断(設定ページ)

### 実装内容(ユーザー指示「次」)
- **`lib/localDiagnostics.ts`(新規)**: 5つのローカルAI連携を並列診断
  - LLM頭脳(Ollama)/ 意味検索埋め込み(nomic-embed-text)/ 音声合成(VOICEVOX →
    speechSynthesis 代替判定)/ 音声入力(whisper)/ 画像生成(SD WebUI)
  - 状態3種: 接続済み(緑)・代替稼働(黄)・未検出(赤)+接続先・失敗理由・
    **有効化ヒント**(ollama pull コマンドや起動手順)
- **設定ページ最上段に「ローカルAI連携の状態(診断)」パネル**: 起動時自動診断+
  🔍再診断ボタン。未検出でも代替手段で業務が止まらないことを明記(自己解決の導線)
### テスト結果
- E2E 8/8: 偽Ollama接続でLLM=接続済み・埋め込み=接続済み(nomic-embed-text)・
  VOICEVOX=代替/ヒント表示・whisper/SD=未検出+💡ヒント・再診断ボタン動作。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第20弾: 予測レポートの保管庫自動保存+CCパネル取込導線

### 実装内容(ユーザー指示「次」)
- **予測レポートの保管庫自動保存**: 深層予測の完了時にレポート全文
  (シナリオツリー・較正・学習ノート・提案込みのMarkdown)を保管庫へ自動保存
  (source: agent / tags: forecast-report)。buildForecastMarkdown を共通化し
  ファイル保存とも同一内容 — チャット「保管庫で予測を探して」で後から引ける完全循環
- **CC保管庫詳細パネルに「資料の取込」**: コマンドセンターから直接テキスト文書を
  取込可能(複数可・Brain記録・RAG索引対象)
### テスト結果
- E2E 5/5: 深層予測実行→保管庫に「未来予測レポート:」(ツリー・較正込み)自動保存→
  チャット「保管庫で未来予測を探して」でヒット→CCパネルから取込→一覧+実体確認。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第19弾: 的中率の週次推移チャート

### 実装内容(ノンストップ続行)
- **`forecastAccuracyTrend`(forecastTracking.ts)**: 判定日ベースの7日ビン×直近8週で
  的中率(hit=1, partial=0.5)を決定論集計(判定なし週は null)
- **市場調査部・的中率セクションに折れ線チャート(インラインSVG)**:
  - 単一系列・検証済みの色 #16A34A(dataviz 6チェック: 明度帯/彩度/コントラストすべて合格)
  - 2pxライン・4px点+2pxサーフェスリング・0/50/100%の控えめグリッド・
    直近値のみ直接ラベル・点ツールチップ(週・的中率・判定件数)
  - **判定なし週は線を切って表示**(ゼロと誤読させない)。判定データが無ければ非表示
### テスト結果
- E2E 6/6: チャート表示・判定週2点(pending除外)・線の分断(2セグメント)・
  直近値100%ラベル・ツールチップ(判定件数)・グリッドラベル。スクリーンショットで
  目視確認。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第18弾: チャット「今日何した?」コマンド

### 実装内容(ユーザー指示「次へ」)
- **`todayDigest.ts` 拡張**: `isTodayDigestQuery`(「今日何した?」「本日の動きは?」
  「今日のサマリー教えて」等を正規化して判定)+`buildTodayDigestReply`
  (📋今日の動き: 実イベント/保管庫/的中率/定例実行/Brain記録+⚠未読の要対応)
- **アシスタントチャット**: 保管庫検索の次に判定を追加(決定論・LLM不要・
  「今日の天気〜」のような通常質問は誤発火しない)
### テスト結果
- E2E 7/7: 「今日何した?」→実イベント2件/保管庫1件(今日1追加)/的中率100%/
  Brain今日1件/⚠承認待ち表示→「本日の動きは?」でも発火→通常質問は誤発火しない。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第17弾: 全社レポートへの実データ統合

### 実装内容(ユーザー指示「次」)
- **`lib/reportRealData.ts`(新規)**: 実データサマリーの集計+Markdown整形(決定論)
  - 🗄 保管庫: 文書数・使用率・出所内訳(取込/企画部/エージェント)
  - ⚖ 予測的中率: 的中率と内訳+較正妥当性(外れ平均実現性が的中以上なら ⚠楽観寄り)
  - 🔔 実イベント: 記録数・未読・要対応
  - ⏰ 定例実行: 有効スケジュール数・実行ログ数・エラー数
- **レポートページ(D-017)**: プレビューの末尾に「実データサマリー(本番機能)」を統合。
  Markdown エクスポートにセクション込み・JSON エクスポートに `realData` オブジェクト込み
### テスト結果
- E2E 8/8: プレビューに保管庫3件(出所内訳)・的中率50%(判定待ち1)・⚠楽観寄り・
  実イベント2件(未読1/要対応1)・定例1件(エラー1)→JSONエクスポート実ファイルに
  realData 検証→Markdownエクスポートにセクション検証。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第16弾: チャット保管庫検索コマンド

### 実装内容(ユーザー指示「次へ」)
- **`lib/vaultSearch.ts`(新規)**: 「保管庫で◯◯を探して/検索して/調べて」をコマンドとして
  解釈(parseVaultSearchQuery)し、保管庫の実文書を直接検索
  - 1) タイトル/本文/タグの**部分一致** — 該当箇所の前後を引用(最大160文字スニペット)
  - 2) ヒットなしなら**意味検索フォールバック**(RAG索引の vault チャンクのみ)
  - 返信は決定論整形(📄タイトル・出所・日付+「引用」+保管庫ページへの案内)で LLM 不要
- **アシスタントチャット**: 承認/実行コマンドの次に保管庫検索を判定(通常の質問は誤発火
  しない)。ヘッダーヒントに「保管庫で◯◯を探して」を明記
### テスト結果
- E2E 6/6: 部分一致ヒット(タイトル・出所・月額980円の引用・案内文)→別表現
  「保管庫から◯◯を検索して」でエージェント文書ヒット→ヒットなしの案内→
  通常質問は誤発火しない。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第15弾: 保管庫のAI司書(整理提案・承認制)

### 実装内容(ノンストップ続行)
- **`lib/vaultCuration.ts`(新規)**: 決定論の整理候補提案+承認後の実行
  - `proposeVaultCuration`: ①同名・同出所の旧版(最新1件を残す) ②30日より古いAI成果物
    (要約済みは除外)を候補化(**提案のみ・データは変更しない**)
  - `summarizeAndRemove`: **候補ごとの承認ボタンから**のみ実行 — 原本を削除し冒頭300文字の
    「要約: <タイトル>」を残す(容量を増やさない順序で処理・本文が短ければ削除のみ)。
    実行は Brain 記録+実イベントに残る
- **保管庫ページ「保管庫の整理(AI司書・承認制)」**: 🧹整理候補を提案 → 候補ごとに
  理由(旧版/古い成果物)表示+「✔承認して要約・削除」/「今回は残す」
### テスト結果
- E2E 8/8: 旧版+40日前AI成果物の2件が候補化(新しい文書は候補外)→「今回は残す」で
  データ無傷→承認で原本削除+要約が残りRAG一覧に表示→実イベント+Brain記録。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第14弾: ワークスペース「今日の動き」実データダイジェスト

### 実装内容(ユーザー指示「次へ」)
- **`lib/todayDigest.ts`(新規)**: 当日0時以降の実データを集計する決定論ダイジェスト
  - 実イベント件数(うち要対応)・保管庫件数(今日の追加)・予測的中率(判定待ち)・
    定例実行(有効件数/今日の実行回数/エラー)・Brain記録(今日)
  - 未読の要対応(warn/error)イベントを最大3件抽出+読み上げ用の一文を生成
- **ワークスペース**: 最上段に「今日の動き(実データ)」セクション — 箇条書きダイジェスト+
  ⚠未読の要対応(通知センターへ遷移ボタン)+🔊読み上げ(VOICEVOX/speechSynthesis)。
  従来のMock部署サマリーは「本日のダイジェスト(部署サマリー)」として下段に維持
### テスト結果
- E2E 8/8: 実イベント2件(要対応1)・保管庫2件(今日1追加)・的中率100%(判定待ち1)・
  定例1件有効今日1回・Brain今日2件・⚠未読要対応表示→通知センターへ遷移。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第13弾: 保管庫のエクスポート/インポート

### 実装内容(ノンストップ続行)
- **`exportVaultJson`**: 保管庫全文書を専用JSON(kind: "vault-export"・バージョン付き)へ
  書き出し(デスクトップは保存ダイアログ・ブラウザはダウンロード)
- **`importVaultJson`**: エクスポートJSONのマージ取込
  - 同一IDは**重複スキップ**、容量超過分は**上限スキップ**(件数を報告)
  - 1文書4万文字へ切り詰め・source不明値は "upload" へ正規化・壊れたJSON/別種ファイルは
    エラーを返し**既存データは変更しない**
- **保管庫ページUI**: 「📦 全文書をエクスポート(JSON)」+「📥 エクスポートを取込」。
  取込結果(追加/重複/容量スキップ件数)を表示し、Brain へ記録。
  取り込んだ文書は次回の索引更新でRAG検索対象に(別端末へ保管庫を持ち運べる)
### テスト結果
- E2E 8/8: エクスポート(ダウンロード実ファイルの形式検証)→取込(新規2件追加+
  重複1件スキップ+容量超過1件スキップ・出所維持)→壊れたJSONはエラー+既存データ無傷。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第12弾: 通知センターの実イベント化

### 実装内容(ユーザー指示「次へ」)
- **`lib/appEvents.ts`(新規)**: 実イベントストア(musasabi.appEvents・上限100件)
  — level(info/warn/error)・タイトル・詳細・時刻・既読フラグ
- **`notifyOs` にフック**: OSトーストの発生源(エージェント完了・承認待ち・定例実行完了・
  的中率更新・予測対応構築完了)が**OS権限の有無に関わらず**実イベントとして必ず記録される
  — トーストを見逃しても通知センターで追える。承認待ちは level "warn"(要対応)
- **通知センター**: 上段に「実イベント(実データ)」セクション新設 — 未読件数・既読/
  すべて既読・既読も表示トグル・履歴消去。下段は従来のMockシステム通知として明示
### テスト結果
- E2E 9/9: チャット実行→承認待ち(要対応)+実行完了(情報)が実イベント記録→未読2件→
  個別既読→未読1件→すべて既読→未読0件→既読も表示→localStorage実体確認(read/level)。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第11弾: Mission Control 実データ司令塔(AI自律ループパネル)

### 実装内容(ノンストップ続行)
- **Mission Control に「AI自律ループ(実データ)」パネルを新設**: localStorage の実データを
  集計し、AIの自律ループを司令室で一望
  - 🗄 保管庫: 文書数・使用率(RAG索引対象)
  - ⚖ 予測的中率: 的中率%・的中/部分/外れ/判定待ちの内訳
  - ⏰ 定例実行: 有効件数・次回実行時刻
  - 🧠 Brain 記録: 件数・直近の行動
  - 直近のAI成果物(source: agent の保管庫文書)を並記
- 各タイルはクリックで該当ページ(保管庫/市場調査部/スケジューラ/Company Brain)へ遷移
- ループの説明文: 予測→承認実行→成果物(保管庫・RAG)→定例突合(的中率)→次回較正が
  すべて端末内で循環することを明記
### テスト結果
- E2E 7/7: パネル表示・保管庫2件/的中率50%・待ち1/定例1件有効/Brain直近行動・
  AI成果物並記・タイルクリックで保管庫ページ遷移。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第10弾: 予測突合の定例自動実行(スケジューラ統合)

### 実装内容(ノンストップ続行)
- **AgentSchedule に `kind: "forecast_verify"` を追加**(省略時 "agent" で後方互換):
  予測と実績の自動突合を定例タスクとして登録できる
- **runScheduleNow の分岐**: forecast_verify は AgentRuntime を使わず
  **決定論突合(LLM不使用)**で実行 — RSS実データ+社内記録を集めて
  autoVerifyForecastOutcomes → 的中率統計を Company Brain へ記録+OS通知/Webhook通知
  (実績データ・判定待ちが無い場合もその旨を実行ログに残す)
- **スケジューラUI**: テンプレート選択に「予測突合(的中率の自動更新)」を追加、
  一覧に ⚖予測突合 バッジ。既存の 60秒ティッカー+起動キャッチアップにそのまま乗る
  (アプリ起動中は毎時/毎日/毎週で的中率が自動更新され続ける)
### テスト結果
- E2E 7/7: 予測突合の定例登録→⚖バッジ→今すぐ実行→「決定論突合(LLM不使用)」で
  2件突合・的中率50%レポート→pending が hit/miss へ更新(localStorage実体確認)→
  Brain へ定例突合の記録。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第9弾: エージェント成果物・議事録の保管庫自動保存

### 実装内容(ノンストップ続行)
- **`saveAgentDocToVault`(lib/vaultStorage.ts)**: エージェント成果物を保管庫へ自動保存する
  共通ヘルパー(source: "agent")。容量超過などの失敗は握りつぶさず結果を返し、
  呼び出し側が通知・返信文に載せる
- **自動保存の配線(3箇所)**:
  - エージェント実行センター: 実行完了時に最終報告を「実行報告: <目標>」として保存
    (保存結果を完了ノートに表示)
  - 部署AI会議: 議事録全文(発言7回+AI CEO結論)を「議事録: <議題>」として保存
  - チャット実行(Claude Code方式)/ 未来予測の承認実行: 完了報告を同様に保存
    (チャットは返信文に「成果物を保管庫へ保存」を明記)
- 保存された成果物は保管庫ページに出所「エージェント」で一覧され、RAG索引されて
  チャット・エージェント・予測から意味検索で参照できる(自己参照ループの完成)
### テスト結果
- E2E 6/6: チャット実行→承認→完了返信に保管庫保存明記→AI会議開催→保管庫ページに
  「実行報告」「議事録」が出所エージェントで表示→localStorage 実体確認(source: "agent")。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-14 — 本番実装第8弾: 予測の的中率トラッキング(予測×実績の突合+較正フィードバック)

### 実装内容(次の候補として提示済み・ユーザー指示「次へ(ノンストップ)」)
- **`evaluateForecastOutcome`(packages/agent-runtime/forecast.ts)**: 予測文と実績
  (RSS見出し・社内記録)の**決定論突合** — キーワード抽出(漢字/カタカナ/英数の連なり・
  一般語除外)→一致率→ hit(≥60%)/ partial(≥30%)/ miss を提案。単体テスト追加(計18件)
- **`lib/forecastTracking.ts`(新規)**: 予測記録ストア(musasabi.forecastOutcomes・上限30件)
  - 深層予測の実行時に**選出シナリオを pending として自動記録**
  - 自動突合(RSS+社内記録)で判定候補を書き込み、**最終判定は人間が上書き可能**
  - 的中率統計(的中/部分/外れ/判定待ち・hit=1, partial=0.5)と較正妥当性
    (的中平均実現性 vs 外れ平均実現性 — 外れの方が高ければ「楽観寄り」警告)
- **市場調査部「予測の的中率トラッキング」セクション(新規)**: 統計タイル+
  「⚖ 実績データと自動突合」+記録一覧(判定バッジ・突合根拠・手動上書き・削除)
- **較正フィードバック**: 的中率実績の要約(buildAccuracyDigest)を pastDigest へ追記 —
  次回予測の🧠学習ノート生成に反映(予測を重ねるほど実現性見積もりが較正される)
### テスト結果
- 単体18件 pass(hit/partial/miss 判定・一般語除外・空予測のゼロ除算なし)
- E2E 10/10: 判定待ち2件→自動突合→一致予測=的中(一致率80%根拠)・不一致予測=外れ→
  的中率50%表示→楽観警告→手動上書き→深層予測実行で新規 pending 自動記録+学習カード表示。
  0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — 本番実装第7弾: 保管庫(Knowledge Vault)の実装化(実文書ストア+RAG統合)

### 実装内容(次の候補として提示済み・ユーザー指示「次へ」)
- **`lib/vaultStorage.ts`(新規)**: 保管庫を Mock から**実文書ストア**へ
  - テキスト資料(txt/md/csv/json/log/tsv)を localStorage へ実保存(完全ローカル・外部送信なし)
  - 容量管理: 合計40万文字・1文書4万文字(超過時はエラーで保存拒否)
  - `vaultChunksForIndex()`: 文書を約400文字チャンクへ分割し RAG 索引用に提供(分類: vault)
- **保管庫(Knowledge Vault)ページ(新規・Knowledge グループ)**: ファイル取込(複数可)・
  保管資料一覧(出所/サイズ/保存日/冒頭)・絞り込み・削除・容量バー(70%注意/90%危険)
- **RAG統合(brainRag.ts)**: `ensureBrainIndex` が保管庫チャンクを索引へ増分取り込み。
  **削除済み文書のチャンクは索引から自動掃除**(prune)— チャット・エージェント・未来予測が
  保管庫の実文書を意味検索で参照できる
- **Mock置換**: 企画部の「保管庫へ保存」(ページ+CC詳細)が実保存に(重複保存ガード付き)。
  Command Center の保管庫カード/詳細パネルが実データ表示(実プレビュー・実削除)に
### テスト結果
- E2E 12/12: 取込→一覧表示→容量表示→**意味検索が文書本文をヒット**→企画部実保存→重複ガード→
  CCカード実データ→実プレビュー→**チャットが保管庫内容をRAG参照**→削除→**削除後は本文が
  検索に出ない**(取込の行動記録はBrainログとして残る)。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — 本番実装第6弾: AGI深層予測(分岐の精度・粒度向上)

### 実装内容(ユーザー指示「AGIで分岐の精度や粒度を上げていくようにして」)
- **`runForecastDeep`**(forecast.ts 拡張): AGI的な多段推論で予測を強化
  - **粒度**: 各主分岐(3本)を**サブ分岐2本へ展開**(加速側/堅実側)= 2階層のシナリオツリー
    (最大6葉)。倫理フィルタは主分岐・サブ分岐の両方に適用
  - **精度①自己批評**: 批評AI(critic)が各葉の根拠の弱点を指摘し、**補正確率との平均で較正**
    (較正後% と 生成時% を並記・批評ノート表示)
  - **精度②履歴学習**: 前回予測の要約(localStorage・直近3件)と最新データの差分から
    **学習ノート**を生成し次回の較正に反映(予測を重ねるほど賢くなる)
  - **統制**: 提案は `packages/agi` の **Musasabi憲章チェック**(checkAgainstConstitution)を通し、
    適合・承認要否のノートを表示
  - 選出は**葉レベル**(較正後実現性が最大の倫理通過サブ分岐)
- UI: 分岐ツリー表示(主分岐→└サブ分岐・選出葉=緑・倫理除外=赤+理由)、
  🧠前回予測からの学習カード、批評AIノート、憲章チェック行。レポート保存もツリー込みに
- ルールベース頭脳に [SUBFORECAST]/[CRITIQUE]/[CALIBRATE] マーカー追加(LLMなしでも動作)

### テスト結果
- 単体17件 pass(較正計算 66%→60%補正→63%・葉レベル選出・**実現性95%の不正サブ分岐除外→
  健全な55%の葉選出**・履歴ありで学習ノート生成・憲章ノート付与)
- E2E 12/12: 2階層ツリー表示・較正後/生成時の併記・批評ノート・学習カード・葉選出・
  憲章チェック表示・倫理除外・承認→構築実行。0エラー / build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — 本番実装第5弾: 未来予測機能(シナリオ分岐・倫理フィルタ・承認後に構築実行)

### 実装内容(ユーザー指示: 市場調査から半年〜1年先を複数分岐で先読み→現実性の高いものを選出→倫理排除→今できる提案→承認後に構築・実行)
- **`packages/agent-runtime/forecast.ts`(新規)**: 未来予測エンジン
  - 入力: 市場実データ(RSS見出し)+社内RAG(調査記録)
  - **3本の分岐シナリオ**(楽観/主流/抑制)を生成(各: タイトル・半年後・1年後・実現性%)
  - **倫理フィルタ**: 欺瞞・世論操作・プライバシー侵害・危害・差別・違法の兆候を決定論スクリーンで
    検出し、該当分岐を**除外理由つきで排除**(実現性が高くても選出対象にしない)
  - **選出**: 倫理を通過した分岐のうち実現性スコア最大のものを選出(選出根拠を明示)
  - **提案**: 選出シナリオから「現在取り組める内容」を生成(実行は人間の承認後)
  - ルールベース頭脳にも [FORECAST]/[PROPOSE] マーカー対応(LLMなしでも動作)
- **市場調査部ページ「未来予測(半年〜1年・シナリオ分岐)」**: 🔮予測→分岐カード
  (選出=緑・倫理除外=赤+除外理由・実現性%)→💡提案→**「✔ 承認して構築・実行」で
  エージェントが実実行**(ボタン承認は監査ログに記録)→予測レポートのファイル保存・Brain保存

### テスト結果
- 単体15件 pass(倫理除外: 実現性90%の不正分岐が排除され倫理通過の60%が選出されることを検証)
- E2E 8/8: 3分岐表示・**倫理違反分岐(偽情報/世論操作・実現性90%)が除外理由つきで排除**・
  実現性72%の健全分岐が選出・提案2件表示・承認クリック→構築実行完了(LLM報告)・監査記録。0エラー
- build ✅ / 秘密情報スキャン ✅
...
- 定例実行スケジュールを iCalendar(.ics)で書き出し(RRULE=毎時/毎日/毎週)。
  Outlook / Google カレンダーへ取り込める(OAuth不要・完全ローカル)。スケジューラに📅ボタン

### ⑥ RAG索引の拡大(チャット履歴)
- Company Brain の意味検索対象に**アシスタントチャット履歴(相談と回答・直近100件)**を追加。
  過去に聞いたこと・指示したことをアシスタント/エージェントが思い出せる
  ※保管庫(Knowledge Vault)は現状Mock保存のため実文書の索引は保管庫実装後

### ⑦ ローカル画像生成(`lib/imageGen.ts` + マーケ部)
- Stable Diffusion WebUI(AUTOMATIC1111・無料・http://127.0.0.1:7860)を自動検出し、
  マーケティング部で txt2img による**素材の実生成**(生成表示・PNG実ファイル保存・Brain記録)。
  未検出時は導入案内のみ(GPU性能依存と明記)

### テスト結果
- E2E: ICSボタン表示・チャット履歴が意味検索でヒット(相談/回答の内容一致)・
  偽SD WebUIへの実HTTPで画像生成→表示 **4/4 pass**・0エラー
- build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — 本番実装第3弾②③: 部署AI会議(マルチエージェント協調)+タスク別モデルルーティング

### ② 部署AI会議(`packages/agent-runtime/discussion.ts`)
- 営業部長AI・市場調査部長AI・マーケティング部長AI が**別人格(システムプロンプト)のエージェント**
  として同じ議題を2ラウンド議論し、AI CEO(司会)が結論をまとめる `runDiscussion`
- ルールベース頭脳にも [DISCUSS]/[CONCLUDE] マーカー対応(LLMなしでも会議が動く)
- エージェント実行センターに「部署AI会議」セクション: 議題入力→🏛開催→発言タイムライン→
  AI CEO 結論(Company Brain へ意思決定として保存)→📄議事録ファイル保存・🔊読み上げ

### ③ タスク別モデルルーティング
- AgentRuntime に `reportProvider` を追加: **計画・報告=高品質モデル / 行動・観察=高速モデル**
- 「報告用モデル(任意)」設定(頭脳カード)。空なら従来どおり単一モデル
- チャット実行・実行センター・定例実行のすべてに適用

### テスト結果
- 単体13件 pass(会議2件・ルーティング1件を追加)
- E2E: 会議(3人格×2R=6発言・CEO結論・Brain保存)6/6、
  ルーティング(観察=0.5b/計画・報告=7b の実リクエスト検証)4/4。0エラー
- build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — 本番実装第3弾①④: チャット会話メモリ+OSトースト通知

### ① 会話メモリ(文脈保持)
- `chatWithHistory` を agent-runtime に追加(直近の user/assistant やり取り最大8ターンを
  文脈としてLLMへ渡す・単体テスト追加で10件 pass)
- アシスタントチャットのLLM応答が「さっきの件」「それを続けて」など**継続会話**に対応
  (RAG併用・実行指示/承認フローとは共存)

### ④ OSトースト通知
- `lib/osNotify.ts`: デスクトップは Windows 通知センター(tauri-plugin-notification)、
  ブラウザは Web Notification API。権限なしは静かにスキップ
- エージェント完了(チャット実行/実行センター/定例実行)と**チャット実行の承認待ち**で表示 —
  アプリを見ていなくても気づける(Webhook 未設定でも可・外部送信なし)

### テスト結果
- 単体10件 pass(chatWithHistory の文脈伝搬検証)
- E2E: 1通目は文脈なし・**2通目で直前のやり取りがLLMに渡る**(msgs=4/prev=yes)2/2 pass・0エラー
- build ✅ / 秘密情報スキャン ✅

---

## 2026-07-13 — アシスタントチャットを指示実行インターフェース化(Claude Code 方式)

### 実装内容(ユーザー要望「AIアシスタントから Claude Code と同じように次の実行指示を送れるように」)
- **チャットからの実行指示**: 「実行 ◯◯」(または `/run ◯◯`・▶実行ボタン)で
  エージェントが**実際に自律実行**(計画→行動→承認→観察→報告)
  - 指示文からワークフローテンプレートを推定(週次→wf-weekly-report / 新サービス→wf-new-service /
    その他は汎用フロー)
  - **承認ノードはチャット内で停止**し、「承認」と送信するだけで再開(Claude Code の承認フローと同じ)
  - 完了時は ✅最終報告(頭脳名・使用ツール・ステップ数)をチャットに返信、
    Company Brain 保存+Webhook 通知(設定時)
  - 途中経過はステップダイジェストで表示。失敗時もエラーを返信
- 通常の質問は従来どおり LLM+RAG の案内応答(実行と共存)
- クイックボタンに「実行 週次レポートを作成して」を追加。返信は改行保持表示に

### テスト結果
- E2E: 「実行 新サービスを立ち上げて」→ チャット内で**承認待ち停止**(ステップ表示)→
  「承認」送信→**✅実行完了+LLM最終報告+Brain保存**→ 通常質問の応答も共存 **6/6 pass**・0エラー
- build ✅ / 秘密情報スキャン ✅

---

## 2026-07-12 — 本番実装第2弾④⑤⑥: 定期自動バックアップ+資料読み取り+起動キャッチアップ

### ④ 定期自動バックアップ(`lib/autoBackup.ts`)
- 全ローカルデータ(musasabi.*)を、フォルダ選択ダイアログで選んだ保存先へ
  **毎日自動で実バックアップ**(Tauri fs 実書き込み・`musasabi-backup-<日付>.json`)
- 設定画面「データ管理」に有効化トグル/📁フォルダ選択/▶今すぐバックアップ/直近結果表示
  (ブラウザ実行時は非対応の旨を表示)。実行は Company Brain に記録

### ⑤ エージェントの資料読み取り(実ファイル内容)
- エージェント実行センターに「📎 資料を添付(txt/md/csv 最大5件)」
- `summarize` ツールが添付時は**実ファイル内容の要約**に切替(端末内処理・外部送信なし)

### ⑥ 定例実行の起動キャッチアップ
- 起動5秒後に1回キャッチアップ実行 — アプリ未起動中に期限が過ぎた定例・自動バックアップを
  起動直後に回収(以降は従来どおり60秒間隔)

### テスト結果
- E2E: 資料添付→カスタム目標実行→**実ファイル内容(新規契約12件)が要約に反映**・
  期限切れ定例が起動キャッチアップで自動完了 **3/3 pass**、
  設定画面の自動バックアップUI表示 2/2。すべて 0エラー / build ✅ / 秘密情報スキャン ✅
...
### 実装内容(「無課金で本番実装」第3弾)
- **A3 実TTS(`lib/voice.ts`)**: Windows 内蔵音声(SpeechSynthesis API・オフライン・無料)で
  日本語読み上げ。アシスタントチャットの各返答に 🔊、エージェント最終報告に「🔊 読み上げ」ボタン。
  日本語ローカル音声を自動選択。外部送信なし
  ※実STT(音声認識)はローカル whisper 導入が必要なため後続(ボタンはMockのまま明記)
- **A5 実OCR(`lib/ocr.ts` + Vision ページ)**: tesseract(WASM)による**本物の文字認識**。
  日本語+英語。エンジン・言語データ(計約7.5MB)を**アプリに同梱**しオフライン動作・外部送信なし。
  Vision ページに「画像OCR(実文字認識・ローカル)」セクション(画像選択→認識→信頼度/処理時間表示→
  Company Brain へ記録)
- 修正: tesseract コアの .wasm 相対解決が Worker 内で失敗 → WASM埋め込み単一ファイル版
  (tesseract-core-simd-lstm.wasm.js)へ切替

### テスト結果
- E2E(OCR): 実画像(PIL生成 "MUSASABI REPORT 2026")を実認識 — **信頼度90%・0.9秒**・
  Brain記録・TTS API 有効 **5/5 pass**・0エラー
- build ✅ / 秘密情報スキャン ✅
- 注: インストーラは OCR 同梱分(+約7.5MB)増加

---

## 2026-07-12 — 無課金本番化 A2+A4: エージェント定例実行+成果物の実ファイル出力

### 実装内容(「無課金で本番実装」第2弾)
- **`lib/agentSchedule.ts`(新規)**: エージェント定例実行(完全ローカル)
  - 登録した目標を、アプリ起動中に期限到来で **実際に AgentRuntime が自律実行**
    (毎時/毎日/毎週・60秒間隔の監視・多重起動なし)
  - 承認ノードは「登録時の事前承認」フラグ有効時のみ自動続行し、`pre_approved` を監査ログへ記録
    (無効なら承認待ちで停止)
  - 完了時は成果と実行記録を Company Brain へ保存。直近5件の実行ログを保持
- **スケジューラページ**: 「エージェント定例実行(本番・ローカル)」セクション新設
  (登録フォーム・次回予定・▶今すぐ実行・有効/無効・実行ログ+報告プレビュー・📄ファイル保存)
- **`lib/agentTools.ts`(新規)**: RAG差し替え済みツール構築を実行センター/スケジューラで共用化
- **A4 実ファイル出力**: エージェント実行センターの最終報告とスケジュール実行報告を
  **Markdown 実ファイルとして保存**(Tauri=保存ダイアログ / ブラウザ=ダウンロード)
- App.tsx 起動時に `startAgentScheduler()` を開始

### テスト結果
- E2E(スケジューラ実実行): セクション表示・登録・**今すぐ実行→承認ノード自動続行→完了**・
  LLM頭脳表示・報告表示・ファイル保存ボタン・**Company Brain への保存** **7/7 pass**・0エラー
- build ✅ / 秘密情報スキャン ✅

### 制約(正直な記載)
- 定例実行はアプリ(デスクトップ常駐)起動中のみ動作。OSサービス化は本番ロードマップ項目

---

## 2026-07-12 — 無課金本番化 A1: Company Brain 意味検索+RAG(ローカル埋め込み)

### 実装内容(ユーザー承認済み「無課金で本番実装」第1弾)
- **`packages/brain-rag`(新規)**: ローカル埋め込み+ベクトル索引
  - `OllamaEmbeddings`: Ollama 互換 `/api/embeddings`(既定 `nomic-embed-text`・無料・端末内)
  - `HashEmbeddings`: 決定論ハッシュ埋め込み(LLMなしでも語彙ベース検索が動くフォールバック)
  - `VectorIndex`: コサイン類似 topK 検索・増分索引・シリアライズ(localStorage 保存)
  - `buildRagContext`: 検索ヒットをLLMプロンプト用コンテキストへ整形
- **`lib/brainRag.ts`**: Company Brain の行動記録(直近300件)を増分索引・4桁丸めで容量圧縮
- **Company Brain ページ**: 「意味検索(ローカルRAG)」セクション新設(関連度%表示)
- **アシスタントチャット**: LLM応答時に関連社内記録を検索してプロンプトへ注入(**実RAG**)
- **エージェント**: `research_snapshot` ツールを実RAG検索へ差し替え(ヒットなしは従来Mock文言)

### テスト結果
- brain-rag 単体 **5件 pass**(実HTTP埋め込み統合・フォールバック・増分索引・復元)
- E2E 3本: RAG(意味検索UI・実埋め込み・チャットRAG注入)**5/5** / エージェント2本(回帰)
  すべて **ページエラー 0** / build ✅ / 秘密情報スキャン ✅

### 使い方
`ollama pull nomic-embed-text`(無料・約270MB)で実埋め込みに切替。未取得でもハッシュ埋め込みで動作。
...
### 実装内容
- 実機で「未検出(フォールバック)」報告を受け、原因切り分け用の診断をエージェント実行センターに追加
- `OllamaProvider.probe()`: 検出失敗の理由(HTTPステータス/タイムアウト/fetch例外)を返すよう拡張
  (probeタイムアウトは 1.5秒→3秒 に延長)。`detectBrain` が `probeError` として返す
- 頭脳カードに診断行を表示: **接続先URL / 接続経路(ネイティブHTTP or ブラウザfetch)/ 失敗理由**
  + 確認手順(Ollama 起動確認→保存して接続テスト)

### テスト結果
- 単体テスト **9件 pass**(probeError の内容検証を追加)/ E2E 2本 0エラー / build ✅ / 秘密情報スキャン ✅
...
### 実装内容
- **`packages/agent-runtime`(新規)**: エージェント実行ループの中核
  - `LlmProvider` 抽象化: **無料ローカルLLM(Ollama 互換API・http://127.0.0.1:11434)**を
    自動検出して頭脳として差し込み(APIキー不要・課金なし・推論は端末内で完結=外部送信なし)
  - `RuleBasedProvider`: LLM未検出でも同じループが動く決定論フォールバック頭脳
  - `AgentRuntime`: **計画→行動(ローカルツール)→承認(人間ゲート)→観察→報告**の自律ループ。
    ワークフローテンプレート(Intelligence Layer)から実行キューを構築し、
    各行動は `validateDecision` のポリシー検証を通過(実外部変更/課金は遮断)。
    承認ノードで人間承認まで停止・最大ステップ数ガード・監査ログ・Company Brain 書き込み
  - `chatOnce`: アシスタントチャットのLLM頭脳化(1問1答)
- **エージェント実行センター(新ページ・Workflow)**: 頭脳検出カード(URL/モデル設定+接続テスト)、
  目標プリセット(新サービス立ち上げ/週次レポート)+カスタム目標、実行タイムライン(1ステップずつ可視化)、
  承認待ちカード(✔ 承認して再開)、最終報告(説明可能性つき)、監査ログ、Company Brain 保存
- **アシスタントチャットのLLM化**: コマンドセンター右下チャットが Ollama 検出時は実LLM応答、
  未検出/失敗時はルールベース応答へ自動フォールバック
- 修正: ブラウザの `window.fetch` を未束縛のまま既定値に使うと Illegal invocation になる問題
  (アロー関数で包んで解決)

### テスト結果
- agent-runtime 単体テスト **8件 pass**(承認一時停止→再開・ポリシー遮断・maxSteps・
  **Ollama互換HTTPエミュレータとの実HTTP統合**・フォールバック・chatOnce)
- Playwright E2E(2本): ①LLMあり(CORS対応エミュレータ:43111)= 頭脳検出🧠・例示フロー実行・
  承認待ち停止→承認再開・最終報告にLLM出力・Brain保存・監査ログ・部署アクター表示 **7/7 pass**
  ②LLMなし= ルールベース頭脳で同一ループ完走 **4/4 pass**。両方 **ページエラー 0**
- `npm run build` 成功・秘密情報スキャン pass

### 使い方(無料ローカルLLM)
1. https://ollama.com からインストール(無料)
2. `ollama pull qwen2.5:0.5b`(軽量・無料モデル。他モデルも設定画面で変更可)
3. Musasabi を起動 → エージェント実行センターが自動検出(未検出時はルールベースで動作)

---

## 2026-07-10 — UIフィードバック第11弾(アシスタントチャット欄を横長化)

### 実装内容
- コマンドセンター右下の Musasabi アシスタントチャット(`.cc-chat-dock`)を、
  **左サイドバー(180px)の境目から右端まで横長**に変更(left:196px / right:20px / width:auto)
- 横長化に合わせてクイック質問を縦積み→**横並び**(flex-direction:row・幅自動)へ、
  入力欄の高さ・履歴の高さを圧縮(max-height 56vh/22vh)し、横長バーとして収まる形に

### テスト結果
- `npm run build` 成功。Playwright: チャットドックがサイドバー境界〜右端に展開・
  クイック質問4件が横一列・チャットボット応答あり・**ページエラー 0**
