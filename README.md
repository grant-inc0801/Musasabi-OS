...
| 音声入力(STT・whisper) | 実装済み(オプトイン) | ローカル whisper サーバ(無料・下記) |
| 市場調査の実データ(RSS/Atom) | 実装済み(オプトイン) | 公開フィードURLを登録(認証不要) |
| 未来予測(シナリオ分岐・倫理フィルタ) | 実装済み | なし(市場調査部の🔮ボタン) |
| VOICEVOX 高品質音声 | 実装済み(オプトイン) | VOICEVOX(無料)を起動するだけ(自動検出) |
| 定期自動バックアップ | 実装済み | 保存先フォルダを一度選ぶだけ(毎日自動) |
| カレンダー書き出し(.ics) | 実装済み | なし(スケジューラから書き出し) |
| ローカル画像生成(SD) | 実装済み(オプトイン) | Stable Diffusion WebUI を `--api` で起動(GPU推奨) |
| エージェントの資料読み取り | 実装済み | 実行時に txt/md/csv を添付するだけ |
...
| メール通知(SMTP・Secret Center) | 実装済み(オプトイン) | アプリパスワード(OS資格情報ストアに暗号化保管) |

Webhook URL・トークンは端末内(localStorage)のみに保存され、未設定なら外部送信は一切発生しない。
メール送信・カレンダー連携は認証情報の保管設計の確定後に追加予定。

### 音声入力(実STT)のセットアップ(任意・無料)
1. [whisper.cpp](https://github.com/ggml-org/whisper.cpp) の `server` を起動: `whisper-server -m ggml-base.bin --port 8080`
   (OpenAI互換のローカルSTTサーバでも可 — `/v1/audio/transcriptions` を自動フォールバック)
2. アプリのチャット「🎤 音声入力」が自動で本物になる(押して話す→停止で文字起こし)。
   音声は端末内 localhost でのみ処理・外部送信なし。未導入時はボタンに「whisper未検出」と表示
