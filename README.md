...
| 画面 | 内容 |
| --- | --- |
| 営業部(既定) | ダッシュボードを集約。全体KPI・AI社員別KPI(コール結果)を横棒グラフで表示(売上表示なし)+ Learning / Test / AutoCall の三段階コールシステム+日次計画・推奨アクション・リード一覧・通話解析。Test Mode は Mock 架電を操作可能、Learning Mode では日々の作業内容を手動登録して学習させられる。AutoCall は「準備中・承認待ち」で本番実行不可 |
