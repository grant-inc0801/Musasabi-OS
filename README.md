...
管理画面はダークテーマ+部門中心サイドバー構成(D-20260706-004 / -006)。サイドバーは
ミニ経営ダッシュボードとして、**全体サマリー**(総AI社員数・総売上・本日進捗・稼働部門数)
と**部門カード**(AI社員数・進捗率・本日の作業内容・売上・ステータス)を常時表示する
(β版はすべてMock値)。

| 画面 | 内容 |
| --- | --- |
| ダッシュボード | KPI・日次計画・推奨アクション・リード一覧・通話解析(Mock) |
| 営業部 | コールシステム中心。部門サマリー+ Learning / Test / AutoCall の三段階。Test Mode は Mock 架電を操作可能、Learning Mode では日々の作業内容を手動登録して学習させられる。AutoCall は「準備中・承認待ち」で本番実行不可 |
...
- **アバターをクリック** するとミニパネルが開閉する(パネル・入力欄・モード切替は
  ...
  - **アバターサイズの調整**: 小 / 中 / 大 のプリセットまたはスライダー(48〜160px)。
    設定は保存され、アバターのみの常駐表示にも反映される(初期値は「中」)
  - 「メイン管理画面を開く」ボタンでの復帰
